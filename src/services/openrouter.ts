...
export const evaluateContent = async (title: string, transcript: string, targetAudience: string = "Предприниматели, интересующиеся ИИ и автоматизацией"): Promise<any> => {
  ...
  const prompt = `
    Оцени не просто качество контента, а НАСКОЛЬКО ОН РЕЛЕВАНТЕН именно для указанной целевой аудитории в формате YouTube Shorts / Reels.
    Название: ${title}
    Транскрипт: ${transcript}
    Целевая аудитория: ${targetAudience}

    Твоя задача:
    1. Определи, насколько тема ролика действительно интересна этой аудитории.
    2. Оцени, насколько язык, примеры, тезисы и подача подходят этой аудитории.
    3. Оцени, затрагивает ли ролик реальные боли, желания, страхи, цели или триггеры этой аудитории.
    4. Учитывай именно релевантность для аудитории, а не абстрактное "качество" ролика.

    Интерпретация score:
    - 90-100: очень высокая релевантность, аудитория почти наверняка заинтересуется
    - 70-89: хорошая релевантность, аудитории это в целом подходит
    - 40-69: частичная релевантность, есть пересечение, но неочевидное попадание
    - 1-39: слабая релевантность, аудитории это скорее не подходит

    ОБЯЗАТЕЛЬНО: Напиши разбор (evaluation) СТРОГО на русском языке.
    В разборе коротко объясни, почему контент релевантен или нерелевантен именно этой аудитории.

    Ответь ТОЛЬКО в формате JSON:
    {
      "score": число от 1 до 100, где score означает степень релевантности контента для целевой аудитории,
      "evaluation": "краткий разбор на русском языке, почему этот контент релевантен или нерелевантен для данной аудитории",
